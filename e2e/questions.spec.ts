...
const fillCreateQuestion = async (page: Page, questionText: string, answerText: string) => {
...
test("creating a question from the sidebar goes to the new question page", async ({ page }) => {
...
test("importing markdown prefills the create question form", async ({ page }) => {
  ...
  await expect(dialog.getByRole("textbox", { name: "question" })).toHaveValue(`# ${headingText}`);
  await expect(dialog.getByRole("heading", { name: headingText })).toBeVisible();
  await expect(dialog.getByRole("textbox", { name: "answer" })).toHaveValue(answerLine);
  await expect(createSubmit(page)).toBeEnabled();
});
...
test("importing invalid markdown shows an error and leaves the form empty", async ({ page }) => {
  ...
  await expect(notifications(page).getByText("The file must start with a heading.")).toBeVisible();
...
test("creating a markdown question previews it and renders it after save", async ({ page }) => {
  const stamp = Date.now();
...
test("a failed create keeps the dialog and shows an error", async ({ page }) => {
  ...
  await expect(page.getByRole("dialog")).toBeVisible();
  await expect(
    notifications(page).getByText("Could not create the question. Try again later."),
  ).toBeVisible();
  await expect(notifications(page).getByText(questionText).first()).toBeVisible();
...
test("create is disabled while the question is saving", async ({ page }) => {
  ...
  const savingCreate = page.getByRole("dialog").getByRole("button", { name: "Loading Create" });
...
test("creating a question from the empty state goes to the new question page", async ({ page }) => {
...
test("questions belong only to the user who created them", async ({ browser }) => {
  ...
  try {
    ...
    await expect(openedQuestion(ownerPage, questionText)).toBeVisible();
...
test("updating a question from the sidebar goes to the question page", async ({ page }) => {
  ...
  await expect(page.getByRole("textbox", { name: "question" })).toHaveValue(questionText);
  await expect(page.getByRole("textbox", { name: "answer" })).toHaveValue("Original answer");
...
test("updating a question shows a spinner while the question loads", async ({ page }) => {
  ...
  await expect(updateDialog.getByRole("heading", { name: "Update question" })).toBeVisible();
  await expect(updateDialog.getByRole("status", { name: "Loading" })).toBeVisible();
  await expect(updateDialog.getByRole("textbox", { name: "question" })).toHaveCount(0);

  releaseQuestionGet();

  await expect(updateDialog.getByRole("textbox", { name: "question" })).toHaveValue(questionText);
  await expect(updateDialog.getByRole("textbox", { name: "answer" })).toHaveValue(
    "Original answer",
  );
  await expect(updateDialog.getByRole("status", { name: "Loading" })).toHaveCount(0);
});
...
test("importing markdown prefills the update question form", async ({ page }) => {
  ...
  await expect(dialog.getByRole("heading", { name: "Update question" })).toBeVisible();
  await expect(dialog.getByRole("textbox", { name: "question" })).toHaveValue(originalQuestion);
  await expect(dialog.getByRole("button", { name: "From markdown" })).toBeVisible();
...
test("importing invalid markdown shows an error and leaves the update form unchanged", async ({
  ...
  await expect(dialog.getByRole("heading", { name: "Update question" })).toBeVisible();
  await expect(dialog.getByRole("textbox", { name: "question" })).toHaveValue(questionText);
  ...
  await expect(notifications(page).getByText("The file must start with a heading.")).toBeVisible();
  await expect(dialog.getByRole("textbox", { name: "question" })).toHaveValue(questionText);
  await expect(dialog.getByRole("textbox", { name: "answer" })).toHaveValue("Original answer");
  await expect(dialog.getByRole("button", { name: "Update" })).toBeDisabled();
});
...
test("a failed delete restores the question and shows a toast", async ({ page }) => {
...
test("sidebar question opens that question in the main pane", async ({ page }) => {
...
test("reloading a question page fetches that question once", async ({ page }) => {
...
test("sidebar stays visible while a child page chunk is loading", async ({ page }) => {
  await signIn(page);
