...
test("creating a question from the theory header stays on theory", async ({ page }) => {
...
test("creating a question from the empty state stays on theory", async ({ page }) => {
...
test("opening a question adds its id to the search params", async ({ page }) => {
...
test("questions belong only to the user who created them", async ({ browser }) => {
  ...
  try {
    ...
    await expect(otherPage).toHaveURL(new RegExp(`/theory\\?id=${questionId}$`));
